...
export default function NewStudentScreen() {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      Alert.alert("Error", err?.response?.data?.detail ?? "No se pudo crear el estudiante");
    }
  };
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} className="flex-1 bg-slate-50">
      <ScrollView contentContainerClassName="px-4 py-6">
        <Text className="text-lg font-semibold text-gray-800 mb-4">Información del estudiante</Text>
